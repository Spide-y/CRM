...
<link href="../../jquery/bootstrap_3.3.0/css/bootstrap.min.css" type="text/css" rel="stylesheet" charset="utf-8"/>
...
<script type="text/javascript" src="../../jquery/bs_pagination/en.js"></script>
	...
	$(function(){
		...
		$("#addBtn").click(function (){
			...
			/*
			* 操作模态窗口的方式:
			* 	需要操作的模态窗口的jquery对象，调用modal方法，为该方法传递参数show:打开 hide:隐藏
			*
			* */
			//走后台取数据给下拉框
			$.ajax({
		...
		$("#saveBtn").click(function (){
			$.ajax({
				...
				success : function (data){
					if (data.success){
						//清空模态窗口中的数据
						//$("#activityAddForm").reset();此方法不能用（表单的jquery对象没有提供此方法,但是原生js有此方法）
						//所以要将jQuery对象转换为dom对象
						$("#activityAddForm")[0].reset();

						//添加成功后，刷新市场活动信息列表(局部刷新)，关闭模态窗口
						/*
		...
		$("#deleteBtn").click(function (){
			...
			if ($xz.length==0){
				...
				if (confirm("确定删除吗？")){
					//拼接参数
					var param = "";
					//将$xz中的每一个dom对象遍历出来，取value值，就相当于取得了需要删除的记录的id
	...
	/**
	 * 对于所有的关系类型数据库，做前端的分页相关操作的基础组件
	 * 就是pageNo和pageSize
	 * pageNo:页码
	 * pageSize:每页展现的记录数
	 * pageList方法:就是发出ajax请求到后台，从后台取出最新的市场活动信息列表数据通过响应回来的数据，局部刷新市场活动信息列表
	 *
					...
					<form class="form-horizontal" role="form">
						...
						<div class="form-group">
							...
							<div class="col-sm-10" style="width: 81%;">
								<!--textarea:
									1.一定是要以标签对的形式呈现，正常状态下标签对咬紧紧的挨着(中间不能有其他东西)
									2.textarea虽然是以标签对的形式呈现的，但它也是属于表单元素范畴，可以用val()方法取值(而不是html()方法)
								-->
			...
			<div class="btn-toolbar" role="toolbar" style="background-color: #F7F7F7; height: 50px; position: relative;top: 5px;">
				<div class="btn-group" style="position: relative; top: 18%;">
					<!--
						...
						但是没有办法对按钮的功能进行扩充
						未来的实际项目开发，对于模态窗口的操作，一定不要写死在元素当中，应该由自己写js代码来操作
					-->
